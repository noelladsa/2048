...
class EventHandler(object):
	...
	def __add_number(self): #Thoughts: Optimize function by recording vacant spots in a list during collapse? Rand will choose from said list
	...
	def __move_number(self,moveto_y,moveto_x, movefrom_y,movefrom_x):
		...
		if moveto_x != movefrom_x or moveto_y != movefrom_y: #Check that number is not in the same place
			...
			number_moved = True #A number was moved 
			for listener in self.listeners:
				listener.number_moved(movefrom_x,movefrom_y,moveto_x,moveto_y, self.grid[moveto_y][moveto_x])
		return number_moved 
